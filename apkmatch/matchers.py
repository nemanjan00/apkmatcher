...
def _deobfuscated_fqns_from_strings(rec: dict) -> set[str]:
    ...
    Note: stripping `.<anonymous>` suffixes was tried but over-clusters
    lambdas (multiple lambdas from one source class all hash the same).
    Keeping the FQN as-is including any anonymous suffix.
    """
    out = set()
    for s in rec.get("strings", ()):
...
class ImplsPositionLXVote:
    ...
    def propose(self, a, b, mapping):
        ...
        for a_cid, b_cid in mapping:
            ra = a.get(a_cid); rb = b.get(b_cid)
            if not ra or not rb: continue
            ai = ra.get("impls", ()); bi = rb.get("impls", ())
            if len(ai) != len(bi) or not ai: continue
            for at, bt in zip(ai, bi):
                if not at.startswith("LX/") or not bt.startswith("LX/"): continue
                if mapping.get(at) is not None: continue
                if mapping.inverse(bt) is not None: continue
                votes[at][bt] += 1
        for a_cid, vmap in votes.items():
            ...
            conf = min(0.7 + 0.05 * (top_v - second), 0.93)
            yield Candidate(a_cid, top_b, conf, self.id,
                            ("impls_position_lx", top_v, second))
